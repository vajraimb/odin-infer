...
**状态:** Stage 1a ✅ 已突破 — kernel 正确 + 530 GFLOPS,根因确诊并复现
**优先级:** 高 — 这是让 Ornith 接 agent/工具调用的关键瓶颈
**最后更新:** 本轮会话(Stage 1a 正确性 + 性能全部通过)
...
| 路径 | TTFT | ms/token |
|---|---|---|
| 逐 token GEMV(旧) | 122.1 s | 115 |
| Stage 1a 批量 MLP(新) | 86.3 s | 81 |

**≈ 1.42× prefill 提速**,正确性已验证(`2+2 → 4`、`capital of France → Paris`、长文摘要连贯)。

### 为什么只有 1.42×(不是 50×)—— Stage 1b 是下一步

Stage 1a 只批量了 MLP(w1/w3/w2)。本模型是混合注意力,**24 个线性注意力层**的 attention 投影
(`in_qkv` [8192×4096]、`in_z`、`out`)和 8 个全注意力层的 `wq/wk/wv/wo` 仍是**逐 token GEMV**,
这些占了 per-token 成本的大头。MLP 只占 ~70% 权重流量,且 1-simdgroup/tile 的 MMQ 还没充分调优。

**Stage 1b(下一个增量,预期再 2-3×)**:把 attention 投影也批量成 MMQ。难点:
- 线性层 `in_qkv/in_z/in_b/in_a` 可批量,但之后的 **conv1d + delta 递推必须逐 token 顺序**
  (状态依赖),`out` 可批量。即:批量投影 → 逐 token conv+delta → 批量 out。
- 全注意力层 `wq/wk/wv/wo` 可批量,但 attention score/softmax/value 仍逐 token(Stage 1c 才批量)。

### 已知约束(别踩)
- `forward_gpu_batch` 要求 `T % 8 == 0` 且 `T ≤ MAX_BATCH_T(512)`;`engine_forward_batch` 自动分块。
- Q4_K_M 里 **`ffn_down`(w2)/`output`/`attn_qkv` 是 Q6_K**,其余 MLP 是 Q4_K —— `enc_mm` 已按 kind 分派,
  新架构务必确认每种权重的 quant type 都有对应 MMQ。
- 改 kernel 后先跑 `gemm_harness/gemm_test.odin` 回归(F16 GEMM + Q4_K/Q6_K MMQ 正确性)。
