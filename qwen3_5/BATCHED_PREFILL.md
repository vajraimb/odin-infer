...
**状态:** Stage 1b ✅ — 投影全批量(MLP + attention),prefill 2.5×;有状态部分仍逐 token
**优先级:** 高 — 这是让 Ornith 接 agent/工具调用的关键瓶颈
**最后更新:** 本轮会话(Stage 1b 端到端 + 实测 2.5×)
...
| **Stage 1b 批量 MLP + attention 投影** | **48.8 s** | **46** | **2.5×** |

**Stage 1b ≈ 2.5× prefill 提速**,正确性已验证(贪心解码输出与 Stage 1a 逐位一致:
`2+2 → 4`、`capital of France → Paris`、1062-token 长文摘要完全相同)。
agent/tool 的核心痛点(1500-token 工具结果 prefill)从 ~3 分钟降到 ~70 秒。
...
逐 token 的只剩**有状态部分**:全注意力的 score/softmax/value 计算,线性注意力的 conv1d+delta 递推。
这两类是顺序依赖,不能简单批量。继续提速的路径:
- **Stage 1c**:全注意力层批量 attention(flash-style,带 causal mask)—— 把 score/softmax/value 也批量化。
- **MMQ tile 调优**(Claude 的建议,此时收益面已到 ~95% 层):多 simdgroup/tile + threadgroup memory 复用反量化结果。当前是 1-simdgroup/tile。
- **Stage 2**:线性注意力层的 chunked gated delta rule(让递推也能批量 + 状态可重建,解锁 qwen3_5 任意 L 续算)。
